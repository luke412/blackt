...
- (void)getStepCount_NSDate:(NSDate *)date completion:(void(^)(double value, NSError *error))completion
{
    ...
    // Since we are interested in retrieving the user's latest sample, we sort the samples in descending order, and set the limit to 1. We are not filtering the data, and so the predicate is set to nil. HKSampleQuery
    

    NSPredicate *myPredicate = [HealthKitManage predicateForSamplesOneday_NSDate:date];
    HKSampleQuery *query = [[HKSampleQuery alloc] initWithSampleType:stepType predicate:myPredicate limit:HKObjectQueryNoLimit sortDescriptors:@[timeSortDescriptor] resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
        if(error)
        {
            completion(0,error);
        }
...
- (void)getDistance_NSDate:(NSDate *)date   completion:(void(^)(double value, NSError *error))completion
{
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc] initWithSampleType:distanceType predicate:myPredicate limit:HKObjectQueryNoLimit sortDescriptors:@[timeSortDescriptor] resultsHandler:^(HKSampleQuery * _Nonnull query, NSArray<__kindof HKSample *> * _Nullable results, NSError * _Nullable error) {
...
//某一天
+ (NSPredicate *)predicateForSamplesOneday_NSDate:(NSDate *)date{
    NSCalendar *calendar = [NSCalendar currentCalendar];
...
//    NSDate *startDate = [calendar dateByAddingUnit:NSCalendarUnitMonth value:-1 toDate:nowDate options:0];
...
//最近七天
+ (NSPredicate *)predicateForSamples7day_NSDate:(NSDate *)date{
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSDateComponents *components = [calendar components:NSCalendarUnitYear|NSCalendarUnitMonth|NSCalendarUnitDay fromDate:date];
    [components setHour:0];
    [components setMinute:0];
    ...
    NSDate *nowDate = [calendar dateFromComponents:components];
    NSDate *startDate = [calendar dateByAddingUnit:NSCalendarUnitDay value:-6 toDate:nowDate options:0];
    NSDate *endDate   = [calendar dateByAddingUnit:NSCalendarUnitDay value:1 toDate:nowDate options:0];
    NSPredicate *predicate = [HKQuery predicateForSamplesWithStartDate:startDate endDate:endDate options:HKQueryOptionNone];
    return predicate;
}
