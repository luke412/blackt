...
//            [path addLineToPoint:CGPointMake(self.frame.size.width,UULabelHeight+i*levelHeight)];
            [path moveToPoint:CGPointMake(10,UULabelHeight+i*levelHeight)];
            [path addLineToPoint:CGPointMake(WIDTH_lk-20,UULabelHeight+i*levelHeight)];
            [path closePath];
            shapeLayer.path = path.CGPath;
            shapeLayer.strokeColor = [[[UIColor whiteColor] colorWithAlphaComponent:0.7] CGColor];
            shapeLayer.fillColor = [[UIColor whiteColor] CGColor];
            shapeLayer.lineWidth = 1;
            [self.superview.layer addSublayer:shapeLayer];
            
            UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(WIDTH_lk-40,UULabelHeight+i*levelHeight-30 , 80, 30)];
...
- (void)strokeChart
{
    for (int i=0; i<_yValues.count; i++) {
        ...
        if ([[_colors objectAtIndex:i] CGColor]) {
            ...
            _chartLine.strokeColor = [UUColor redColor].CGColor;
        }
